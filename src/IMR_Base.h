...
public:
    ...
    inline size_t get_PBA(const size_t &LBA) { return LBA_to_PBA.count(LBA) ? LBA_to_PBA[LBA] : LBA_to_PBA[LBA] = -1; }
    inline size_t get_LBA(const size_t &PBA) { return PBA_to_LBA.count(PBA) ? PBA_to_LBA[PBA] : PBA_to_LBA[PBA] = -1; }
