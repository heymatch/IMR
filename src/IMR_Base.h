...
namespace IMR_Base{
    ...
    static std::unordered_map<size_t, size_t> LBA_to_PBA;
    static std::unordered_map<size_t, size_t> PBA_to_LBA;
    static std::vector<bool> track_written;
    ...
    static inline size_t get_PBA(const size_t &LBA) { return LBA_to_PBA.count(LBA) ? LBA_to_PBA[LBA] : LBA_to_PBA[LBA] = -1; }
    static inline size_t get_LBA(const size_t &PBA) { return PBA_to_LBA.count(PBA) ? PBA_to_LBA[PBA] : PBA_to_LBA[PBA] = -1; }
    static inline void set_LBA_PBA(const size_t &LBA, const size_t &PBA) { LBA_to_PBA[LBA] = PBA; PBA_to_LBA[PBA] = LBA; }
    static inline size_t get_track(const size_t &PBA){
        if (PBA == -1)
		return -1;
    ...
    static inline size_t get_track_head(const size_t &track){
        return track % 2 ? 
